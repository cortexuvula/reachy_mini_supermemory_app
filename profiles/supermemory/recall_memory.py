...
async def _resolve_recall_tags() -> List[str]:
    ...
    Order: explicit env pin → discovered minus excludes → own-scope fallback.
    The fallback ensures we never send an empty containerTags array, which the
    API rejects.
    """
    ...
    if discovered:
        excluded = set(recall_excluded_tags())
        kept = [t for t in discovered if t not in excluded]
        if kept:
            return kept
